...
import sys
sys.path.append('../../end2end_generation')
from src.download_metrics.bleu2 import compute_bleu
...
def language_evaluate_mask_with_sig(re, mask=None, dataset_name=None):
    ...
    re['bleu_1'] = []
    for i in range(len(re['content_pred_tokens'])):
        re['bleu_1'].append(bleu_class(re['content_true_tokens'][i][0],re['content_pred_tokens'][i]))
    return re
...
def get_iterate_results(path_name, print_log = False, ):
    ...
    if print_log:
        output_str = f"corpus_bleu_score_1: {np.mean(result['corpus_bleu_score'][1]):.4f} rouge_1: {np.mean(result['rouge_scores']['rouge-1']['r']):.4f} rouge_l: {np.mean(result['rouge_scores']['rouge-l']['r']):.4f} valid_loss: {np.mean(result['valid_loss']):.4f} wer: {np.mean(result['wer']):.4f} bleu_1: {np.mean(result['bleu_1']):.4f}"
        print(output_str)
    return result
