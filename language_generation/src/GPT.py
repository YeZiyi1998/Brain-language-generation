...
class GPT_Tokenizer():
    ...
    def encode_plus(self, words, max_length=None, truncation=True, padding='max_length'):
        if type(words) is str:
            words = words.split()
        if max_length is None:
            max_length = len(words)
        # 将单词转换为id
        input_ids = [self.word2id[x] if x in self.word2id else self.UNK_ID for x in words]
        # 创建attention_mask
